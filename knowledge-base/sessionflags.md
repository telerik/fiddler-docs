...
+ If you examine `oFlags["non-existent-flag"]`, the result is `null`.
...
+ While you can call `oFlags.Add("flagname")`, this throws an exception if the flag already exists. It is better to set the value: `oFlags["flagname"] = "value";`
...
+ Note: hiding a session frees up the memory that is otherwise used to hold the session data in memory.
...
If present, this session's entry is bolded in the Session List.

`ui-italic`

If present, this session's entry is italicized in the Session List.

`ui-strikeout`

If present, this session's entry is struck out in the Session List.
...
If present, execution of this session pauses before the request is issued.

`x-breakresponse`

If present, execution of this session pauses after the response is received.

### Host Flags

`x-overrideHost`

Provide the Host:Port combination to use for DNS resolution purposes. Note that this mechanism does not change the HOST header on the request, and thus is not useful if there is an upstream gateway.
...
Provide the Host:Port combination of a gateway to use to proxy this request, or DIRECT to send the request directly to the origin server.
...
If set on a CONNECT tunnel, the traffic in the tunnel is not decrypted.  Requires Fiddler Classic v2.0.8.9 or later.

`https-Client-Certificate`

Filename of client certificate (for example, `.cer`) to attach to this secure request. Requires Fiddler Classic v2.1.0.3 or later.

`x-OverrideCertCN`

String specifying the hostname to appear in the CN field of this CONNECT tunnel's Fiddler-generated certificate.
