...
page_title: Out-of-Memory Exception - Fiddler Classic
description: Address out-of-memory errors in Fiddler Classic by adjusting buffer limits and session handling.
slug: OutOfMemory
publish: true
position: 10
---
...
Fiddler Classic works by storing the entire request and response in memory. If you are performing a huge download (hundreds of megabytes), it is possible that Fiddler Classic cannot find a free memory block large enough to hold the entire contiguous response, and hence you will run into this "out of memory" problem. It is also possible that if you have thousands of sessions in the Fiddler Classic session list, even a relatively small memory block will not be available to store a response a few megabytes in size. You can reduce the incidence of this problem by clearing the **Web Sessions** list (`Ctrl+X`) or configuring it to automatically trim to the most recent two hundred sessions (click the **Filters** tab, and click the **Keep only the most recent sessions** option at the bottom).

For more details about this issue, refer to the [Out of Memory explanation](https://blogs.msdn.com/ericlippert/archive/2009/06/08/out-of-memory-does-not-refer-to-physical-memory.aspx) and the [Large Object Heap improvements](https://blogs.msdn.com/b/dotnet/archive/2011/10/04/large-object-heap-improvements-in-net-4-5.aspx).
...
If you are using [FiddlerCore](http://fiddler2.com/core) or writing a Fiddler Classic Extension, you can use code like this:
```js
