...
function apiNavigationTemplate(root) {
    return function (data) {
        var item = data.item;
        var text = item.text;
        ...
        if (url.indexOf("#") < 0) {
            ...
            //     url = item.path + "/" + url;
            // }
            return formatNavigationTemplate(icon, root + "api/" + url, text);
        } else {
            return formatNavigationTemplate(icon, url, text);
        }
    };
...
$(function () {
    ...
    $("pre").each(function (index) {
        var langExtension = codeSampleMapper[$(this).attr('data-lang')];
